  ...
  try {
    ...
    if (checkoutError) {
      if (isCheckoutFunctionMissing(checkoutError)) {
        await manualCheckout(actor.supabase, checkoutPayload, orderItemsPayload);
      } else {
        throw appError(ERR.SERVER_ERROR, {
...
async function manualCheckout(
  ...
  const { error: orderError } = await supabase.from("orders").insert({
    ...
    channel: checkoutPayload.channel as "pos" | "reseller", // Type assertion for generated types
    reseller_id: checkoutPayload.reseller_id,
    payment_method: checkoutPayload.payment_method as "cash" | "transfer",
    payment_status: checkoutPayload.payment_status as "paid" | "unpaid" | "void",
    status: checkoutPayload.status as "open" | "paid" | "void" | "refunded",
